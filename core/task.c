...
void eos_schedule() {
	...
	if(highest_priority != 63){
		...
		if(val == NULL) {return;}
			...
			PRINT("%d next task's stack top pointer address : %p\n", _os_current_task->priority, _os_current_task->stack_pointer);
...
void _os_wakeup_single(_os_node_t **wait_queue, int32u_t queue_type) {
	//PRINT("wakeup_single\n");
	eos_tcb_t* waiting_task = (*wait_queue)->ptr_data;
	PRINT("wait queue : %p\n", (*wait_queue));
	_os_remove_node(wait_queue, &(waiting_task->node));
	PRINT("wait queue : %p\n", *wait_queue);
	int32u_t priority = waiting_task->priority;
	//PRINT("waiting task : %p\n", waiting_task);
	PRINT("waiting task priority : %d\n", priority);
